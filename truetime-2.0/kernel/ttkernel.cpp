...
void TT_RUNKERNEL_ERROR(const char *error_msg) {
  ...
  mexCallMATLABWithTrap(0, NULL, 1, rhs, "error");
}
        ...
        if (task->execTime / rtsys->cpuScaling < TT_TIME_RESOLUTION) {
          ...
          if (task->codeFcnMATLAB == NULL) {
            ...
            *mxGetPr(rtsys->segArray) = (double)task->segment;
            ...
            if (mexCallMATLABWithTrap(2, lhs, 2, rhs, task->codeFcnMATLAB) != 0) {
              rtsys->error = true;
              return 0.0;
            }
...
#include "simstruc.h"
  ...
  static void mdlInitializeSizes(SimStruct *S)
  {
    ...
    *((void **)mxGetData(rtsys->rtsysptr)) = (void *)rtsys;
    ...
    if (mexCallMATLABWithTrap(1, lhs, 1, rhs, "nargin") != 0) {
      goto error;
    }
    nargin = (int)*mxGetPr(lhs[0]);
    if (nargin == 0) {
      if (mexCallMATLABWithTrap(0, NULL, 0, NULL, initfun) != 0) {
        goto error;
      } else {
      ...
      if (mexCallMATLABWithTrap(0, NULL, 1, rhs, initfun) != 0) {
        goto error;
      } else {
        rtsys->init_phase = false;
      }
    ...
    if (rtsys->error) {
      error:
        mexCallMATLABWithTrap(1 ,error_msg_array, 0, NULL, "lasterr");
...
#define MDL_ZERO_CROSSINGS
  ...
  static void mdlTerminate(SimStruct *S)
  {
    ...
    mexCallMATLABWithTrap(0, NULL, 2, rhs, "clear");
#endif
  }
